...
5. Separate one-shot axis commands, continuous autoscale modes, manual limits, and restore-range commands. Preserve the opposite axis explicitly for X-only and Y-only commands.
6. Preserve manual wheel, pan, entered, and restored limits across data refreshes. Never call full `AutoScale()` unconditionally in every data update.
...
11. Validate at multiple DPI settings, with live updates, restore-range precedence, and representative maximum-size data.
...
- Read [references/interaction-and-axes.md](references/interaction-and-axes.md) when implementing autoscale, range restore, wheel/pan persistence, double-click, crosshairs, context menus, or manual XY ranges.
...
- Disable continuous auto modes after wheel zoom, drag pan, one-shot adjustment, manual limit entry, or range restore unless the product explicitly requests otherwise.
- Restore an explicit per-chart saved range before falling back to fixed limits from the chart configuration. Validate the chosen range, apply both axes, synchronize state, persist if needed, and refresh once.
...
- Restore prefers the last explicitly saved range, falls back to the configured fixed range, turns off continuous X/Y, and survives the next data refresh.
